...
def test_embedding_shape():
    ...
    layer = Embedding(num_embeddings=vocab_size, embedding_dim=embedding_dim)
    y = layer(x)
...
def test_embedding_forward_matches_pytorch():
    ...
    vocab_size = 100
    embedding_dim = 32
    x = torch.tensor([[1, 5, 7, 10], [2, 5, 8, 10]])
    reference = nn.Embedding(num_embeddings=vocab_size, embedding_dim=embedding_dim)
    layer = Embedding(num_embeddings=vocab_size, embedding_dim=embedding_dim)
...
def test_embedding_repeated_indices():
    vocab_size = 10
    embedding_dim = 8
    x = torch.tensor([[3, 3, 5, 7]])
    layer = Embedding(num_embeddings=vocab_size, embedding_dim=embedding_dim)
    y = layer(x)
    loss = y.sum()
    loss.backward()
    expected = torch.full((embedding_dim,), 2.0)
    torch.testing.assert_close(layer.weight.grad[3], expected)
    expected = torch.ones(embedding_dim)
    torch.testing.assert_close(layer.weight.grad[5], expected)

def test_embedding_dtype():
    for dtype in [torch.float32, torch.float64]:
        layer = Embedding(num_embeddings=100, embedding_dim=32, dtype=dtype)
...
def test_embedding_cuda():
    ...
    layer = Embedding(num_embeddings=1000, embedding_dim=64, device=device)
...
def test_embedding_parameter():
    layer = Embedding(num_embeddings=1000, embedding_dim=64)
    assert isinstance(layer.weight, nn.Parameter)
    assert layer.weight.requires_grad
    assert layer.weight.shape == (1000, 64)